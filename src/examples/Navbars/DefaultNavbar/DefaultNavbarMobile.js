...
function DefaultNavbarMobile({ routes, open }) {
  ...
  const renderNavbarItems = routes.map(
    ({ name, icon, collapse: routeCollapses, href, route, collapse: navCollapse }) => (
      ...
      >
        <SoftBox sx={{ height: "auto", maxHeight: "15rem", overflowY: "scroll" }}>
          {routeCollapses &&
            routeCollapses.map((item) => (
              <SoftBox key={item.name} px={2}>
                {item.collapse ? (
                  <>
                    <SoftBox width="100%" display="flex" alignItems="center" p={1}>
                      <SoftBox
                        display="flex"
  ...
  return (
    <Collapse in={Boolean(open)} timeout="auto" unmountOnExit>
      <SoftBox width="calc(100% + 1.625rem)" my={2} ml={-2}>
        {renderNavbarItems}
      </SoftBox>
    </Collapse>
  );
